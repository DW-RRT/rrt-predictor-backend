...
def _dataset()->Dict[str,Any]:
    row=fetch_one("""SELECT COUNT(*) AS runner_rows, COUNT(*) FILTER (WHERE actual_position IS NOT NULL) AS completed_runner_rows,
        ...
        COUNT(DISTINCT (meeting_id::text||'|'||COALESCE(race_number::text,''))) FILTER (WHERE actual_position IS NOT NULL AND model_version IN ('2.18.3','2.18.4','2.19.0','2.19.1','2.19.2','2.19.3','2.19.4','2.19.5')) AS native_completed_races,
...
def _promote(cycle_id:str, current:Dict[str,Any], proposed:Dict[str,float], gate:Dict[str,Any])->Dict[str,Any]:
    ...
    new_id=f"2.19.5-auto-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    ...
    execute_sql("""INSERT INTO rrt_model_weight_sets(model_version,status,weights_json,source,notes,activated_at,promoted_by_cycle_id,promotion_evidence_json,automatic_promotion)
       VALUES(%s,'Active',%s::jsonb,'Adaptive Learning','Automatically promoted after all v2.19.5 safety gates passed.',NOW(),%s,%s::jsonb,TRUE);""",
...
def run_adaptive_learning_cycle(cycle_name:str="v2.19.5 autonomous adaptive learning cycle",save_result:bool=True)->Dict[str,Any]:
    try:
      ...
      sim=run_weight_simulation(test_weights=proposed,simulation_name="v2.19.5 adaptive promotion candidate",notes="Automatic promotion-gate validation.",save_result=True,simulation_group="v2.19.5 promotion gate")
