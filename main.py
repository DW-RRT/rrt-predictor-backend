...
from database_manager import (
    ...
    save_runner_factor_snapshots as save_runner_factor_snapshots_to_postgres,
    ...
    integrate_speed_ratings_into_factor_snapshots,
)
...
from selection_intelligence import (
    run_selection_intelligence_analysis,
    get_latest_selection_analysis,
    get_selection_analysis_history,
    get_top_misses,
    get_factor_impact_report,
...
def api_route_check():
    ...
    required_routes = {
        ...
        "/api/reports/systems": True,
        "/api/reports/systems-html": True,
        ...
        "/api/speed-rating/integrate": True,
        ...
        "/api/model/promotion-status": True,
        "/api/model/candidates": True,
        "/api/model/run-promotion-cycle": True,
    ...
    return {
        ...
        "selection_intelligence_routes_available": all(
            route_availability.get(route)
            for route in [
                "/api/selection-intelligence/run",
                "/api/selection-intelligence/latest",
                "/api/selection-intelligence/history",
...
# ---------------------------------------------------------------------
# Historical Profile Intelligence Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Performance Reporting Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
@app.get("/api/reports/systems")
def api_report_systems():
    return get_systems_report()


@app.get("/api/reports/systems-html", response_class=HTMLResponse)
def api_report_systems_html():
    return generate_systems_report_html()


# ---------------------------------------------------------------------
# Performance Analytics Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Learning Centre Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Factor Capture Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
async def api_speed_rating_backfill(
    ...
    return await asyncio.to_thread(backfill_speed_ratings_from_saved_results, batch_size)



@app.get("/api/speed-rating/integrate")
def api_speed_rating_integrate():
    return integrate_speed_ratings_into_factor_snapshots()
...
# ---------------------------------------------------------------------
# Evidence-Based Factor Analysis Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
def api_analysis_weight_recommendations():
    ...
    if isinstance(report, dict):
        report["recommendation_version"] = "2.22.2"
        ...
        report["safety_note"] = (
            ...
            "They are evaluated through the v2.22.2 Promotion Controller; production weights change only when the configured promotion gates pass and live mode is authorised."
        )
    return report
...
# ---------------------------------------------------------------------
# Historical Weight Simulation Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------

@app.get("/api/simulator/run")
def api_simulator_run(
    simulation_name: str = Query("v2.22.2 analysis-only simulation"),
    notes: str = Query(""),
...
async def api_simulator_run_default_suite(
    min_meeting_date: Optional[str] = Query(None),
    max_meeting_date: Optional[str] = Query(None),
):
...
def api_model_promotion_audit(limit: int = Query(20)):
    ...
    return {"success":True,"model_version":"2.22.2","audit_count":len(rows),"audits":rows}
...
def api_model_candidates(limit: int = Query(20)):
    return get_candidate_history(limit=limit)


@app.post("/api/model/run-promotion-cycle")
def api_model_run_promotion_cycle(
    cycle_id: Optional[str] = Query(None),
    candidate_name: str = Query("v2.22.2 autonomous promotion candidate"),
):
...
# ---------------------------------------------------------------------
# Selection Intelligence Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------

@app.get("/api/selection-intelligence/run")
def api_selection_intelligence_run(
    min_meeting_date: Optional[str] = Query(None),
    max_meeting_date: Optional[str] = Query(None),
):
    return run_selection_intelligence_analysis(
        min_meeting_date=min_meeting_date,
        max_meeting_date=max_meeting_date,
        save_result=True,
    )
...
@app.get("/api/selection-intelligence/latest")
def api_selection_intelligence_latest():
    return get_latest_selection_analysis()


@app.get("/api/selection-intelligence/history")
def api_selection_intelligence_history(limit: int = Query(10)):
    return get_selection_analysis_history(limit=limit)


@app.get("/api/selection-intelligence/top-misses")
def api_selection_intelligence_top_misses(limit: int = Query(25)):
    return get_top_misses(limit=limit)


@app.get("/api/selection-intelligence/factor-impact")
def api_selection_intelligence_factor_impact():
    return get_factor_impact_report()
...
# ---------------------------------------------------------------------
# Native Adaptive Learning Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------

@app.get("/api/adaptive-learning/run")
def api_adaptive_learning_run(
    cycle_name: str = Query("v2.22.2 autonomous adaptive learning cycle"),
):
...
# ---------------------------------------------------------------------
# Historical Replay Engine Routes - RRT Predictor v2.22.2
# ---------------------------------------------------------------------

@app.get("/api/replay/run")
def api_replay_run(
    replay_name: str = Query("v2.22.2 production-versus-candidate replay"),
    min_meeting_date: Optional[str] = Query(None),
    max_meeting_date: Optional[str] = Query(None),
    ...
    return run_historical_replay(
        ...
        model_version=model_version, save_result=True,
...
# ---------------------------------------------------------------------
# Punting Form Results / Accuracy helpers (RRT Predictor v2.22.2)
# ---------------------------------------------------------------------
...
def _save_prediction_snapshot(
    ...
    factor_save_result = save_runner_factor_snapshots_to_postgres(snapshot)

    snapshot["postgres_history"] = postgres_save_result
    snapshot["factor_capture_history"] = factor_save_result
...
def _compare_prediction_to_results(
    ...
    return {
        "success": True,
        "provider": "Punting Form",
        "source": "RRT Predictor v2.22.2 Single-Factor Historical Simulation Suite",
...
# ---------------------------------------------------------------------
# Trifecta result audit / backfill - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
def _backfill_trifecta_result_history(limit: int = 100) -> Dict[str, Any]:
    """
    Populate missing v2.22.2 meeting-level Trifecta outcomes from already-saved
...
# ---------------------------------------------------------------------
# Automatic Results Processor - RRT Predictor v2.22.2
# ---------------------------------------------------------------------
...
def _process_single_meeting_results(
    ...
    if not prediction_snapshot:
        requested_model_version = str(model_version or "2.22.2")
        ...
        if not postgres_prediction.get("success"):
            return {
                "success": False,
                "provider": "RRT Predictor",
                "source": "RRT Predictor v2.22.2 Automatic Results Processor",
                "meeting_id": meeting_id,
...
def _run_autonomous_control_if_due(results_run: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        learning = run_adaptive_learning_cycle(
            cycle_name="v2.22.2 autonomous adaptive learning cycle",
            save_result=True,
        )
        ...
        promotion = run_promotion_cycle(
            cycle_id=cycle_id,
            candidate_name="v2.22.2 autonomous adaptive promotion candidate",
            save_result=True,
        )
    ...
    try:
        ...
        if prediction_response.get("success"):
            ...
            prediction_response["prediction_history"] = {
                ...
                "note": "v2.22.2 persistence status is reported from the actual PostgreSQL save response; runner-factor capture is reported separately.",
            }
        ...
        # v2.22.2 retained optimisation: refresh profile caches after the compact
