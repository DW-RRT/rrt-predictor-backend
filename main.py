...
# ---------------------------------------------------------------------
# Performance Reporting Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Performance Analytics Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Learning Centre Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Factor Capture Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Evidence-Based Factor Analysis Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
def api_analysis_weight_recommendations():
    ...
    if isinstance(report, dict):
        report["recommendation_version"] = "2.20.0a"
        ...
        report["safety_note"] = (
            ...
            "production weights. Automatic promotion is disabled in v2.20.0a and Normalised Speed is active at the manually approved 10% production weight."
        )
    return report
...
# ---------------------------------------------------------------------
# Historical Weight Simulation Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------

@app.get("/api/simulator/run")
def api_simulator_run(
    simulation_name: str = Query("v2.20.0a analysis-only simulation"),
    notes: str = Query(""),
...
def api_model_promotion_audit(limit: int = Query(20)):
    ...
    return {"success":True,"model_version":"2.20.0a","audit_count":len(rows),"audits":rows}
...
# ---------------------------------------------------------------------
# Selection Intelligence Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Native Adaptive Learning Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------

@app.get("/api/adaptive-learning/run")
def api_adaptive_learning_run(
    cycle_name: str = Query("v2.20.0a autonomous adaptive learning cycle"),
):
...
# ---------------------------------------------------------------------
# Historical Replay Engine Routes - RRT Predictor v2.20.0
# ---------------------------------------------------------------------

@app.get("/api/replay/run")
def api_replay_run(
    replay_name: str = Query("v2.20.0a production-versus-candidate replay"),
    min_meeting_date: Optional[str] = Query(None),
    max_meeting_date: Optional[str] = Query(None),
...
# ---------------------------------------------------------------------
# Punting Form Results / Accuracy helpers (RRT Predictor v2.20.0)
# ---------------------------------------------------------------------
...
def _compare_prediction_to_results(
    ...
    return {
        "success": True,
        "provider": "Punting Form",
        "source": "RRT Predictor v2.20.0a Single-Factor Historical Simulation Suite",
...
# ---------------------------------------------------------------------
# Automatic Results Processor - RRT Predictor v2.20.0
# ---------------------------------------------------------------------
...
def _process_single_meeting_results(
    ...
    if not prediction_snapshot:
        requested_model_version = str(model_version or "2.20.0")
        ...
        if not postgres_prediction.get("success"):
            return {
                "success": False,
                "provider": "RRT Predictor",
                "source": "RRT Predictor v2.20.0 Automatic Results Processor",
                "meeting_id": meeting_id,
    ...
    try:
        ...
        if prediction_response.get("success"):
            ...
            prediction_response["prediction_history"] = {
                ...
                "note": "v2.20.0a persistence status is reported from the actual PostgreSQL save response; runner-factor capture is reported separately.",
            }
