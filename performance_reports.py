...
def generate_learning_report_html() -> str:
    ...
    html = [
        ...
        '<div class="note">This section compares completed runner factor scores against actual results. <strong>Current Automated Factor %</strong> is the active PostgreSQL production weight currently used by the automated prediction model. Production weights can change only through an authorised Promotion Controller decision.</div>',
        '<h3>Factor Effectiveness Ranking</h3>',
        _html_table(['Rank','Factor','Current Automated Factor %','Winner Gap','Place Gap','Win Corr','Place Corr','Signal','Confidence','Recommendation'], [[i.get('predictive_rank'),i.get('label'),next((r.get('current_weight') for r in ((report.get('weight_recommendations') or {}).get('recommendations') or []) if r.get('factor') == i.get('factor')), ''),i.get('winner_gap'),i.get('place_gap'),i.get('win_correlation'),i.get('place_correlation'),i.get('signal_strength'),i.get('confidence'),(i.get('recommendation') or {}).get('direction')] for i in ((report.get('factor_effectiveness') or {}).get('factors') or [])[:13]]),
...
def generate_learning_report_pdf_bytes() -> bytes:
    ...
    story.append(Paragraph("This section compares completed runner factor scores against actual results. Current Automated Factor % is the active PostgreSQL production weight currently used by the automated prediction model. Production weights can change only through an authorised Promotion Controller decision.", styles["BodyText"]))
    factor_effectiveness = report.get("factor_effectiveness") or {}
    weight_recommendations = report.get("weight_recommendations") or {}
    current_factor_weights = {i.get('factor'): i.get('current_weight') for i in (weight_recommendations.get('recommendations') or [])}
    story.append(Paragraph("Factor Effectiveness Ranking", styles["RRTHeading"]))
    story.append(t(["Rank","Factor","Current Automated Factor %","Win Gap","Place Gap","Win Corr","Place Corr","Signal","Conf"], [[i.get('predictive_rank'),i.get('label'),current_factor_weights.get(i.get('factor'), ''),i.get('winner_gap'),i.get('place_gap'),i.get('win_correlation'),i.get('place_correlation'),i.get('signal_strength'),i.get('confidence')] for i in (factor_effectiveness.get('factors') or [])[:13]], [0.8*cm,2.6*cm,2.3*cm,1.6*cm,1.6*cm,1.6*cm,1.6*cm,1.8*cm,1.5*cm]))
