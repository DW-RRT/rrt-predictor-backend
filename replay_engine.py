...
DEFAULT_WEIGHTS: Dict[str, float] = {
    ...
    "distance_record": 0.09, "track_distance": 0.09,
    "track_condition": 0.12, "trainer": 0.10, "jockey": 0.08,
    "trainer_jockey": 0.12, "barrier": 0.04, "weight": 0.02,
    "market": 0.03,
}
FACTOR_COLUMNS = {
    "last10": "last10_score", "win_place": "win_place_score",
    "track_record": "track_record_score", "distance_record": "distance_record_score",
    "track_distance": "track_distance_record_score", "track_condition": "track_condition_score",
    "trainer": "trainer_score", "jockey": "jockey_score",
    "trainer_jockey": "trainer_jockey_score", "barrier": "barrier_score",
    "weight": "weight_score", "market": "market_score",
}
...
def _metrics(groups: Dict[Tuple[Any, Any], List[Dict[str, Any]]], score_key: str, roughie_min_price: float, roughie_min_rank: int) -> Dict[str, Any]:
    race_count = top1_hits = top4_hits = top4_place_hits = roughie_hits = roughie_races = 0
    selections: List[Dict[str, Any]] = []
    for key, runners in groups.items():
        ranked = sorted(runners, key=lambda r: (_float(r.get(score_key)), -_float(r.get("market_price"), 9999)), reverse=True)
        winner = next((r for r in runners if int(_float(r.get("actual_position"), 999)) == 1), None)
        if not ranked or not winner:
            continue
        race_count += 1
        top4 = ranked[:4]
        top1_hit = ranked[0].get("runner_key") == winner.get("runner_key")
        top4_hit = winner.get("runner_key") in {r.get("runner_key") for r in top4}
        top4_place_hit = any(1 <= int(_float(r.get("actual_position"), 999)) <= 3 for r in top4)
        top1_hits += int(top1_hit)
        top4_hits += int(top4_hit)
        top4_place_hits += int(top4_place_hit)
        roughies = [r for r in ranked if _float(r.get("market_price")) >= roughie_min_price and int(_float(r.get("market_rank"), 99)) >= roughie_min_rank]
        roughie_hit = False
        if roughies:
            roughie_races += 1
        ...
        selections.append({
            "meeting_id": key[0], "race_number": ranked[0].get("race_number"),
            "track": ranked[0].get("track"), "meeting_date": ranked[0].get("meeting_date"),
            "top_selection": ranked[0].get("runner_name"), "winner": winner.get("runner_name"),
            "top1_hit": top1_hit, "top4_win_hit": top4_hit, "roughie_hit": roughie_hit,
        })
    pct = lambda hits, total: round(hits / total * 100.0, 2) if total else 0.0
    return {
        "race_count": race_count,
        "top1_win_hits": top1_hits, "top1_win_strike_rate": pct(top1_hits, race_count),
        "top4_win_hits": top4_hits, "top4_win_strike_rate": pct(top4_hits, race_count),
        "top4_place_hits": top4_place_hits, "top4_place_strike_rate": pct(top4_place_hits, race_count),
...
def run_historical_replay(replay_name: str = "v2.18.1 historical replay", test_weights: Optional[Dict[str, Any]] = None,
    ...
    include_selections: bool = False) -> Dict[str, Any]:
    try:
        ...
        improvement = {key: round(_float(replay.get(key)) - _float(current.get(key)), 2) for key in [
            "top1_win_strike_rate", "top4_win_strike_rate", "top4_place_strike_rate", "roughie_win_strike_rate"]}
        ...
        result = {
            ...
            "production_weights_changed": False, "model_version": model_version or "all_full_field_models",
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "dataset": {
                "runner_count": len(rows), "race_count": len(groups),
                "meeting_count": len({r.get('meeting_id') for r in rows}),
                "min_meeting_date": min(dates) if dates else None,
                "max_meeting_date": max(dates) if dates else None,
            ...
            "recommendation": "Use only full-field pre-race captures. Legacy selected-runner rows are excluded.",
        }
        if include_selections:
            result["current_selections"] = current["selections"]
            result["replay_selections"] = replay["selections"]
        if save_result:
            execute_sql("""INSERT INTO rrt_replay_runs
                ...
                (replay_id,replay_name,REPLAY_VERSION,model_version,min(dates) if dates else None,max(dates) if dates else None,
                 len(rows),len(groups),len({r.get('meeting_id') for r in rows}),json.dumps(DEFAULT_WEIGHTS),json.dumps(weights),
                 json.dumps(result["current_metrics"]),json.dumps(result["replay_metrics"]),json.dumps(improvement),json.dumps(result,default=str)))
        return result
...
def get_replay_report(replay_id: Optional[str] = None) -> Dict[str, Any]:
    row = fetch_one("SELECT replay_json FROM rrt_replay_runs WHERE replay_id=%s;" if replay_id else "SELECT replay_json FROM rrt_replay_runs ORDER BY created_at DESC LIMIT 1;", (replay_id,) if replay_id else ())
    return (row or {}).get("replay_json") or {"success": False, "replay_version": REPLAY_VERSION, "message": "No replay run found."}


def get_replay_history(limit: int = 20) -> Dict[str, Any]:
    rows = fetch_all("""SELECT replay_id,replay_name,replay_version,model_version,min_meeting_date,max_meeting_date,
...
def get_replay_summary() -> Dict[str, Any]:
    row = fetch_one("""SELECT COUNT(*) AS replay_count,MAX(created_at) AS latest_replay_at,
