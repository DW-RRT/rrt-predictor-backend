...
def get_freshness_first_up_analysis() -> Dict[str, Any]:
    """Analyse days since each horse's previous cached start. Analysis-only; production weight remains 0%."""
    try:
        rows = fetch_all("""
            WITH completed AS (
                SELECT DISTINCT ON (
                    ...
                    COALESCE(runner_id::TEXT, UPPER(TRIM(runner_name)))
                )
                    meeting_id,
                    meeting_date,
                    race_id,
                ...
                WHERE actual_position IS NOT NULL
                  ...
                  AND (runner_id IS NOT NULL OR NULLIF(TRIM(runner_name), '') IS NOT NULL)
                ORDER BY
                    meeting_id,
                    COALESCE(race_id::TEXT, race_number::TEXT, ''),
                    COALESCE(runner_id::TEXT, UPPER(TRIM(runner_name))),
            ...
            prior AS (
                SELECT
                    c.*,
                    CASE
                        WHEN c.runner_id IS NOT NULL THEN (
                            SELECT MAX(h.run_date)
                            FROM rrt_horse_history h
                            WHERE h.horse_id = c.runner_id
                              AND h.run_date < c.meeting_date
                        )
                        ELSE (
                            SELECT MAX(h.run_date)
                            FROM rrt_horse_history h
                            WHERE UPPER(TRIM(h.horse_name)) = UPPER(TRIM(c.runner_name))
                              AND h.run_date < c.meeting_date
                        )
                    END AS previous_run_date
                FROM completed c
            ),
            banded AS (
                SELECT
            ...
            grouped AS (
                SELECT
                    ...
                    ROUND(
                        100.0 * COUNT(*) FILTER (WHERE actual_position = 1)
                        / NULLIF(COUNT(*), 0),
                        2
                    ) AS win_pct,
                    ROUND(
                        100.0 * COUNT(*) FILTER (WHERE actual_position BETWEEN 1 AND 3)
                        / NULLIF(COUNT(*), 0),
                        2
                    ) AS place_pct
                FROM banded
            ...
            SELECT *
            FROM grouped
            ORDER BY CASE freshness_band
                WHEN '0-14 days' THEN 1
                WHEN '15-28 days' THEN 2
                WHEN '29-60 days' THEN 3
                WHEN '61-120 days' THEN 4
                WHEN '121+ days / first-up-like' THEN 5
                ELSE 6
            END;
        """)
        return {
            ...
            "analysis": "freshness_first_up",
            "analysis_only": True,
            "production_weight": 0.0,
            "production_model_changed": False,
            "bands": rows,
            "note": (
                "Freshness/First-Up is evidence gathering only. "
                "Distinct completed starts are analysed; it is not included in "
                "production scoring or adaptive promotion candidates."
            ),
        }
    except Exception as error:
        return {
